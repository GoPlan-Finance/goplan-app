...
export class HoldingHelper {
  ...
  private static async updateHolding (holding : Holding) {
    ...
    const transactions = transactionsStore.transactions.filter(transaction => {
      ...
      console.log('PONTERS', transaction.symbol.__type, holding.symbol.__type)
    ...
    if (!holding.currency) {
      if (holding.symbol) {
        holding.currency = holding.symbol.currency
      } else if (transactions.length && transactions[0].currency) {
        holding.currency = transactions[0].currency
      }
    }
    ...
    transactions.forEach(transaction => {
      console.log(transaction.type, transaction.quantity)
