...
export class HoldingTimeSeriesHelper {
    private static dateGenerator(holding: Holding, transactions: Transaction[], period: SymbolDataResolution) {
        if (transactions.length === 0) {
            return;
        }

        const start = HoldingTimeSeriesHelper.getStartOfPeriod(dayjs(transactions[0].executedAt), period);
        const end = HoldingTimeSeriesHelper.getEndOfPeriod(dayjs(transactions[transactions.length - 1].executedAt), period);
    ...
    public static getStartOfPeriod(date: dayjs.Dayjs, period: SymbolDataResolution): dayjs.Dayjs {
        ...
        if (['minute', 'hour', 'day', 'month', 'year'].includes(period)) {
    ...
    private static getOrCreate(
        ...
        price.period = SymbolDataResolution.DAY;
    ...
    public static async updateHistory(holding: Holding, transactions: Transaction[]): Promise<void> {
        ...
        const period = SymbolDataResolution.DAY
        ...
        for (const date of dates) {
            ...
            lastElem = elem;
            const price = HoldingTimeSeriesHelper.getOrCreate(holding, existingPrices, date.executedAt, period);
