<template>
  <div class="grid grid-cols-2 gap-2">
    ...
    >
      ...
      <label
        v-for="[key, filter] in Object.entries(filters).filter(
          ([key, filter]) => (alignment === 'right' && !filter.align) || filter.align === alignment
        )"
        :key="key"
      >
  ...
  <div class="grid gap-2 px-4 py-2 text-gray-400 text-sm" :style="tableTemplate">
    <div v-for="(row, rowIndex) in tableLayout" :key="rowIndex" class="grid items-center">
      <div
  ...
  props: {
    config: {
      ...
      validator: (config: TableConfig) => {
        if (config.search && config.search.handler && typeof config.search.handler !== 'function') {
          return false;
        }
