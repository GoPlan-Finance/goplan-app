...
export class Canada_NationalBank_CDBN extends BaseCSVImporter {
  ...
  private computeValue<T extends keyof CsvDataInterface>(
    name: keyof CsvDataInterface,
    rawData: unknown
  ): CsvDataInterface[T] {
  ...
  private getTransformer(colName: string) {
    const transformer = Object.entries(this.transformers).find(([name, transformer]) =>
      transformer.columns.includes(colName)
    );
