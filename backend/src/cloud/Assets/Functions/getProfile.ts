...
Parse.Cloud.define('Assets--GetProfile', async request => {
  ...
  const industry = !apiProfile.industry
    ? null
    : await CacheableQuery.create(AssetIndustry).findOrCreate(
        {
  ...
  const sector = !apiProfile.sector
    ? null
    : await CacheableQuery.create(AssetSector).findOrCreate(
        {
  ...
  const addressRegion = !apiProfile.country
    ? null
    : await CacheableQuery.create(AssetAddressRegion).findOrCreate(
        {
