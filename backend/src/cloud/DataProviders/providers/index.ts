...
for (const provider: ProviderConfigInterface of global.weHateGlobals_dataProviders) {
  ...
  if (provider.name.toLowerCase() === 'eod') {
    providers.push(new EOD(provider.apiKey));
  }
}
...
class GlobalProvider {
  ...
  static async handleThrottle<T>(
    provider: Types.DataProviderInterface,
    fn: () => Promise<T>
  ): Promise<T> {
  ...
  async getSymbolTimeSeriesData(
    ...
    return GlobalProvider.adjustResolution(resolution, result.resolution, result.data);
  }

  async getCompanyProfile(assetSymbol: AssetSymbol): Promise<Types.CompanyProfile> {
    const provider = GlobalProvider.getProviderFor(assetSymbol);
    return GlobalProvider.handleThrottle(provider, async () => {
      return await provider.getCompanyProfile(assetSymbol.tickerName);
    });
  }
