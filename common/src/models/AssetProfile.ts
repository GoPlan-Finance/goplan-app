...
export class AssetProfile extends BaseObject {
  ...
  static async fetchProfile(symbol: AssetSymbol): Promise<AssetProfile> {
    const profilePointer = (await Parse.Cloud.run('Assets--GetProfile', {
      assetSymbolId: symbol.id,
    })) as Parse.Pointer;

    return CacheableQuery.create(this)
      .equalTo('symbol', symbol)
      .include(['exchange', 'industry', 'symbol', 'industry', 'sector', 'addressRegion'])
      .first();
  }
}
